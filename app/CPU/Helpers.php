...
{
    // دعم لكل من الـ array و object
    $type   = data_get($product, 'product_type', '');
    $unit   = data_get($product, 'unit_value', 1);
    $discountType = data_get($product, 'discount_type', '');
    $discount     = (float) data_get($product, 'discount', 0);
    ...
    return $unit > 0 ? ($discount / $unit) : 0;
}
public static function discountCalculatePrice($product, float $price, int $unit): float
{
    $type        = data_get($product, 'product_type', '');
    $discountType = data_get($product, 'discount_type', '');
    $discount     = (float) data_get($product, 'discount', 0);
    $unitValue    = (int)   data_get($product, 'unit_value', 1);
...
{
    $type       = data_get($product, 'product_type', '');
    $taxId      = data_get($product, 'tax_id', null);
    $purchase   = (float) data_get($product, 'purchase_price', 0);
    $sell       = (float) data_get($product, 'selling_price', 0);
    $unitValue  = (int) data_get($product, 'unit_value', 1);
    $discount   = (float) data_get($product, 'discount', 0);
    $discountType = data_get($product, 'discount_type', '');

    $taxRate = optional(\App\Models\Taxe::find($taxId))->amount ?? 0;

    if ($type === 'service') {
        // الخدمة ليس لها وحدة أو خصم بالوحدة
        return $price * ($taxRate / 100);
    }

    $applyDiscount = !in_array($price, [$purchase, $purchase / $unitValue]);

    if ($applyDiscount) {
        $price_discount = $discountType === 'percent'
            ? ($price * $discount) / 100
            : ($unitValue > 0 ? ($discount / $unitValue) : 0);
    } else {
        $price_discount = 0;
    }

    if ($price === $sell || $price === $purchase) {
        return $price * ($taxRate / 100);
    }

    return ($price - $price_discount) * ($taxRate / 100);
}
public static function tax_calculate_after($product, $price)
{
    $type    = data_get($product, 'product_type', '');
    $taxId   = data_get($product, 'tax_id', null);
    $taxRate = optional(\App\Models\Taxe::find($taxId))->amount ?? 0;
    ...
    public static function vehilceCode($id)
    {
        $vehicle = Store::where('store_id', $id)->first();
        return optional($vehicle)->store_code;
    }
}
//for translation
