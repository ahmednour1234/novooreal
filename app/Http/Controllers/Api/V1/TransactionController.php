...
class TransactionController extends Controller
{
    ...
    public function transferAccounts(Request $request): JsonResponse
    {
        ...
        if (isset($request->customer_balance)) {
            $accounts = $this->account->orderBy('id')->where('type', 0)->paginate($request['limit'], ['*'], 'page', $request['offset']);
            $data = [
                'limit' => $limit,
                'offset' => $offset,
            ...
            $accounts = $this->account->orderBy('id')->where('type', 0)->latest()->paginate($request['limit'], ['*'], 'page', $request['offset']);
            $data = [
