...
{
    // Fetch all distinct customer data from the customers table that have entries in the seller_customers table
    $allCustomers = Customer::join('seller_customers', 'customers.id', '=', 'seller_customers.customer_id')
        ->select('customers.*')
    ...
    $data = [
        'total' => $allCustomers->count(), // Count the total number of results
...
public function getSearch(Request $request): JsonResponse
{
    ...
    $search = $request->name;
    $sellerId = Auth::id(); // Assuming you can get the current authenticated seller's ID
    
// Get all customer_id values where seller_id = $sellerId
$customerIds = SellerCustomer::where('seller_id', $sellerId)->pluck('customer_id')->toArray();

// Return the count of customer IDs


// Retrieve all customers from the customers table where id is in the customer_id array
$result = Customer::whereIn('id', $customerIds)
...
$data = [
    'total' => $result->count(), // Total count of matching customers
    'limit' => null, // Indicate no limit
    'offset' => $offset,
    'customers' => $result, // The list of customers
    'count' => $result->count(), // Total count of customers
];
