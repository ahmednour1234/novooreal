...
class CategoryController extends Controller
{
    ...
    public function getIndex(Request $request): JsonResponse
    {
        ...
        {
            $cat_ids = SellerCategory::where('seller_id', Auth::user()->id)->pluck('cat_id');

            $categories = $this->category->whereIn('id', $cat_ids)->paginate($limit, ['*'], 'page', $offset);
        }
        $data =  [
            'total' => $role == 'seller' ? count($categories) : $categories->total(),
            'limit' => $limit,
            'offset' => $offset,
