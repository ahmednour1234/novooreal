...
class AccountController extends Controller
{
    ...
    public function getIndex(Request $request): JsonResponse
    {
        ...
        $accounts = $this->account->where('type',0)->get();
        $data = [
    ...
    public function getSearch(Request $request): JsonResponse
    {
        ...
        $search = $request->name;
        if (!empty($search)) {
            $result = $this->account->where('account', 'like', '%' . $search . '%')->where('type',0)->orWhere('account_number', 'like', '%' . $search . '%')->latest()->paginate($limit, ['*'], 'page', $offset);
            $data = [
                'total' => $result->total(),
                'limit' => $limit,
                'offset' => $offset,
