...
.custom-select:focus, .custom-input:focus {
    border-color: #4CAF50;
    box-shadow: 0 0 5px rgba(0, 200, 81, 0.7);
}
        ...
        <div class="row align-items-center mb-3">
            <div class="col-sm mb-2 mb-sm-0">
                <h1 class="page-header-title d-flex align-items-center g-2px text-capitalize"><i
                        class="tio-files"></i> {{\App\CPU\translate('القيود المحاسبية')}}
...
<form action="{{ url()->current() }}" method="GET">
    <div class="row m-1">
        ...
        <div class="form-group col-12 col-sm-6 col-md-3 col-lg-3">
            <label class="input-label font-weight-bold" for="account_id">{{ \App\CPU\translate('الحساب') }}</label>
            <select id="account_id" name="account_id" class="form-control js-select2-custom custom-select">
        ...
        <!-- Branch Dropdown -->
        <div class="form-group col-12 col-sm-6 col-md-3 col-lg-3">
            <label class="input-label font-weight-bold" for="branch_id">{{ \App\CPU\translate('الفرع') }}</label>
            <select id="branch_id" name="branch_id" class="form-control js-select2-custom custom-select">
                <option value="">---{{ \App\CPU\translate('اختار') }}---</option>
        ...
        <div class="form-group col-12 col-sm-6 col-md-3 col-lg-3">
            <label class="input-label font-weight-bold" for="cost_id">{{ \App\CPU\translate('مركز التكلفة') }}</label>
            <select id="cost_id" name="cost_id" class="form-control js-select2-custom custom-select">
                <option value="">---{{ \App\CPU\translate('اختار') }}---</option>
                @foreach ($costcenters as $costcenter)
                    <option value="{{ $costcenter['id'] }}" {{ $cost_id == $costcenter['id'] ? 'selected' : '' }}>
                        {{ $costcenter['name'] }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group col-12 col-sm-6 col-md-3 col-lg-3">
            <label class="input-label font-weight-bold" for="tran_type">{{ \App\CPU\translate('النوع') }}</label>
            <select id="tran_type" name="tran_type" class="form-control js-select2-custom custom-select">
                <option value="">---{{ \App\CPU\translate('اختار') }}---</option>
...
@if ($acc_id!=null || $tran_type!=null || $from!=null || $to!=null)
        ...
        <!-- From Date -->
        <div class="form-group col-12 col-sm-6 col-md-3 col-lg-3">
            <label class="input-label font-weight-bold" for="start_date">{{ \App\CPU\translate('من تاريخ') }}</label>
            <input id="start_date" type="date" name="from" class="form-control custom-input" value="{{ $from }}">
        </div>

        <!-- To Date -->
        <div class="form-group col-12 col-sm-6 col-md-3 col-lg-3">
            <label class="input-label font-weight-bold" for="end_date">{{ \App\CPU\translate('الي تاريخ') }}</label>
            <input id="end_date" type="date" name="to" class="form-control custom-input" value="{{ $to }}">
        </div>

        <!-- Search Buttons -->
        <div class="col-12">
            <div class="row d-flex justify-content-center">
                <button class="btn btn-success col-3 mr-1 custom-btn">{{ \App\CPU\translate('بحث') }}</button>
                <a href="{{ route('admin.account.list-transection') }}" class="btn btn-danger col-3 mr-1 custom-btn">
                ...
                <a href="{{ route('admin.account.transection-export',['account_id'=>$acc_id,'tran_type'=>$tran_type,'from'=>$from,'to'=>$to]) }}" 
                   class="btn btn-info col-3 custom-btn" data-toggle="tooltip" data-placement="top" title="{{ $chk == 0 ? \App\CPU\translate('export_last_month_data') : '' }}">
                    {{ \App\CPU\translate('اصدار في اكسل شيت') }}
                </a>
            </div>
        </div>
                    ...
                    <!-- Table -->
                        <button class="btn btn-primary final none" onclick="printTable()">تقرير</button>
...
@endphp
 <table border="1" style="width: 100%; border-collapse: collapse;">
        <tr>
            ...
            <td colspan="3" style="text-align: center; font-weight: bold;">
 {{$account->account ?? 'كل الحسابات'}}
</td>
        </tr>
    </table>
        <div class="table-responsive datatable-custom" id="product-table">
<table class="table table-bordered table-thead-bordered table-striped table-hover table-align-middle card-table">
    <thead class="thead-light">
        <tr>
            <th rowspan="2">{{ \App\CPU\translate('رقم القيد') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('التاريخ') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('الحساب') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('مركز التكلفة') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('بواسطة') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('الفرع') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('النوع') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('الوصف') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('العملة') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('المدين') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('الدائن') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('الرصيد') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('طباعة') }}</th>
            <th rowspan="2">{{ \App\CPU\translate('قيد عكسي') }}</th>
        </tr>
    </thead>
        ...
        @foreach ($transections as $key => $transection)
            <tr>
                ...
                <td>
                    <div class="account-container">
                        <div class="account-item">
                            {{ $transection->costcenter ? $transection->costcenter->name : ' ' }}
                        </div>
                        <div class="account-item">
                            {{ $transection->costcenter_to ? $transection->costcenter_to->name : ' ' }}
                        </div>
                    </div>
                </td>
                ...
                <td>
                    ...
                    @elseif ($transection->tran_type == 500)
                        <span class="badge badge-danger">تحويل مندوب</span>
                    ...
                    @elseif($transection->tran_type == 13)
                        <span class="badge badge-soft-warning">سداد مشتريات</span>
                ...
                <td>
                    <div class="account-container">
                        <div class="amount-item text-danger">
                            {{ number_format($transection->debit, 2) }}
                        </div>
                        <div class="amount-item text-success">
                            {{ number_format($transection->credit, 2) }}
                        </div>
                    </div>
                </td>
                <td>
                    <div class="account-container">
                        <div class="amount-item">
                            {{ number_format($transection->balance, 2) }}
                        </div>
                        <div class="amount-item">
                            {{ number_format($transection->balance_account, 2) }}
                        </div>
                    </div>
                </td>
                <td>
...
@elseif($transection->tran_type == 4 || $transection->tran_type == 7 || $transection->tran_type == 12 || $transection->tran_type == 24 ||  $transection->tran_type == 3)
    <span class="badge badge-warning">
...
<div class="modal fade" id="reverseConfirmationModal" tabindex="-1" role="dialog" aria-labelledby="reverseConfirmationModalLabel" aria-hidden="true">
...
<div class="modal fade" id="reverseResultModal" tabindex="-1" role="dialog" aria-labelledby="reverseResultModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
...
<script>
    function printTable() {
        ...
        // Pass supplier data from PHP to JavaScript
         const customerName = @json($account ? $account->account : null);
